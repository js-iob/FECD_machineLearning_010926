...
# Ensure editable text in Illustrator
...
df = pd.read_csv(
    "D:/IOB/Projects/ocularDiseases_ML/Sep_2025/1_expressionProfiles/FECD/expressionProfile_fecd_adjustedCount_090225_transpose.txt",
    sep="\t"
)
...
print(f"Total genes: {X_log.shape[1]}")
...
# ============================================================
# STEP 3: 5-fold CV feature selection
# ============================================================
...
print("\n=== Cross-validation Performance (Linear SVM) ===")
cv_auc = []
cv_acc = []
cv_f1 = []
cv_sens = []
...
for fold, (train_idx, val_idx) in enumerate(
    cv.split(X_train_full, y_train_full),
    start=1
):
    ...
    # --------------------------------------------------------
    # Scale BEFORE feature selection
    # --------------------------------------------------------

    scaler_fs = StandardScaler()

    X_train_scaled_fs = scaler_fs.fit_transform(X_train_cv)
    X_val_scaled_fs = scaler_fs.transform(X_val_cv)

    # --------------------------------------------------------
    # Linear SVM feature selection
    # Use stronger regularization and smaller panel
    # --------------------------------------------------------
    ...
    selected_features_fold = X_train_cv.columns[mask]

    print(
        f"Selected genes in fold {fold}: "
        f"{len(selected_features_fold)}"
    )

    selected_feature_sets.append(
        set(selected_features_fold)
    )
    ...
    # --------------------------------------------------------
    # Same kernel for classification
    # --------------------------------------------------------
    '''
    model = SVC(
        kernel='linear',
...
# Keep genes appearing in >=4 folds
# ============================================================
...
print("\nStable genes:", len(stable_features))

with open("svmSelected_features.txt", "w") as out:
    for gene in stable_features:
        out.write(gene + "\n")
...
X_train_final = X_train_full[stable_features]
...
with PdfPages("SVM_ROC.pdf") as pdf:
    ...
    plt.plot(
        ...
        color='darkorange',
        lw=2,
        label=f'Test ROC curve (AUC = {roc_auc:.2f})'
    )
...
with PdfPages("SVM_CM.pdf") as pdf:
    ...
    sns.heatmap(
        ...
        fmt='d',
        cmap='Blues',
        xticklabels=['Control','FECD'],
        yticklabels=['Control','FECD']
    )
